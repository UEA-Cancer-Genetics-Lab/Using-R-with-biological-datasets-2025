...
#### Tasks
1. When the growth rate is restricted to 0.05, what is the average expression level across all genes in the “response to stress” biological process, separately for each limiting nutrient? 
2. What about genes in the “protein biosynthesis” biological process? 
_Hint:_ 3 pipes: `filter`, `group_by`, `summarise`.
...
- `head(n=10)` gives the top `n` rows of a data frame.

#### Tasks
1. Which 10 biological process annotations have the most genes associated with them?  _Hint:_ 4 pipes: `group_by`, `summarise` with `n_distinct`, `arrange`, `head`.
...
library(rstatix)
library(ggpubr)
dat_16S <- read_csv( "https://github.com/UEA-Cancer-Genetics-Lab/Using-R-with-biological-datasets-2025/blob/a51b10274d020b15453e6ee0ab7f6e7963327bd5/data/Arcanobacterium_16S.csv")
```


#### Tasks
1. Determine whether there is a differential abundance for the abundance of Arcanobacterium between aggressive prostate cancer (H) and clinically benign (CB) patients, using a Welch *t* test.
2. Create a plot
