---
title: "Using R with biological datasets 2025"
...
IQR(iris$Petal.Length, na.rm = TRUE)
```
...
library(rstatix)
library(ggpubr)
dat_16S <- read_csv( "https://raw.githubusercontent.com/UEA-Cancer-Genetics-Lab/Using-R-with-biological-datasets-2025/refs/heads/main/data/Arcanobacterium_16S.csv")
...
#2
tukey_res <- cow_diet |> 
  tukey_hsd(uracil ~ diet) |> 
  add_xy_position() |> 
  add_significance()

ggplot(cow_diet, aes(x=diet, y=uracil)) + 
  geom_boxplot() + 
  stat_pvalue_manual(tukey_res, label = "p.adj.signif")
```
...
```{r}
xtabs(~island+sex, data=penguins)
```
...
Example:
```{r}
chisq.test(xtabs(~island+sex, data=penguins))
```


```{r}
prop.table(xtabs(~island+sex, data=penguins), margin=1)
```
```{r}
...
#1
xtabs(~sex+status, data=lung_new)

#2
chisq.test(xtabs(~sex+status, data=lung_new))
...
survfit2(Surv(time, status)~sex, data=lung) |> 
  ...
  scale_color_manual(values = c('#54738E', '#82AC7C'), name = "Sex", labels = c("Male", "Female")) +
  scale_fill_manual(values = c('#54738E', '#82AC7C'), name = "Sex", labels = c("Male", "Female")) +
  theme_minimal() +
  ...
  labs(
    ...
    x = "Days to event",
    title = "Kaplan-Meier plot of survival times for lung cancer patients grouped by gender"
  ) 
```
...
fit <- coxph(Surv(time,status)~
               sex+age+ph.ecog+ph.karno+pat.karno+meal.cal+wt.loss, data=lung) |>
tidy(exponentiate=TRUE, conf.int=TRUE)
